...
static double slct (void* vv) {
  ...
  for (i=0,n=0;i<nk;i++) if (key[i]>=EQV && key[i]<=EQX) n++; 
  if (nk+n!=num) { 
...
static double mredundout (void* vv) {
  ...
  if (indflag) { 
    ...
    for (j=0;j<num; j++) vector_resize((IvocVect*)vva[j], ns-k); 
