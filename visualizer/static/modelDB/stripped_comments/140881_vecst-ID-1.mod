...
static double indset (void* vv) {
  ...
  if (equ) {
    ...
    if (ifarg(3)) { 
      ...
      if (inc) inc=val2; else for (i=0; i<nx; i++) { x[i]=val2; } 
    }
    for (i=0; i<ny; i++) {
...
static double mkind(void* vv) {
  ...
  y[0]=0.; y[ny-3]=nx; y[ny-2]=min; y[ny-1]=max; 
...
static double sindx (void* vv) {
  ...
  nx=av[0]; 
  for (i=0;i<num;i++) { 
    bv[i]=list_vector_px(ob2, i, &bvo[i]); 
  if (bv[i]!=ni) { 
...
static double slct (void* vv) {
  ...
  for (j=0;j<nk;j++) { 
    ...
    if (key[j]<=EBE && key[j]>=ALL) { field[j]=0;
      ...
      if (key[j]<=EBE*(m+1) && key[j]>=ALL*(m+1)) { 
        key[j]/=(m+1);
        field[j]=m;
      }
    }
...
static double slor (void* vv) {
  ...
  for (j=0,k=0,m=0;j<ni;j++) { 
    for (i=0,m=0,n=0,fl=0;i<num;i++,n++,m+=2) { 
      if (field[n]==0) val=vvo[i][j]; else UNCODE(vvo[i][j],field[n],val);
      if (key[n]==ALL) {fl=1; break;} 
...
static double iwr (void* vv) {
  ...
  nx = vector_instance_px(vv, &x);
  scrset(nx);
  for (i=0;i<nx;i++) scr[i]=(int)x[i]; 
  if(!ifarg(2) || !((int)*getarg(2))) r=fwrite(&nx,sizeof(int),1,f);  
...
static double mredundout (void* vv) {
  ...
  maxsz=vector_buffer_size((IvocVect*)vv);
  if (indflag) vector_resize((IvocVect*)vv, maxsz); 
  num = ivoc_list_count(ob);
  ...
  for (j=0;j<num;j++) val[j]=avo[j][0]; 
  for (i=1,k=0;i<ns;i++) { 
    for (j=0,match=1;j<num;j++) {
      if (val[j]!=avo[j][i]) { match=0; break; } 
    }
    if (match) { 
      if (k>=scrsz){printf("mredundout****ERRD**** over scr size %d\n",k);hxe();}
      scr[k++]=i; 
    } else for (j=0;j<num;j++) val[j]=avo[j][i]; 
  }
  if (indflag) { 
    ...
    for (i=0,p=scr[0]; i<k-1; i++) { 
      for (m=scr[i],p--; m<scr[i+1]; m++,p++) { 
        for (j=0;j<num; j++) avo[j][p]=avo[j][m]; 
        for (j=0;j<numb;j++) bvo[j][p]=bvo[j][m]; 
      }
    }
    for (m=scr[i],p--; m<ns; m++,p++) { 
      for (j=0;j<num; j++) avo[j][p]=avo[j][m]; 
      for (j=0;j<numb;j++) bvo[j][p]=bvo[j][m]; 
    }
    for (j=0;j<num; j++) vector_resize(vva[j], ns-k); 
...
static double nearall (void* vv) {
  ...
  for (kk=0,i=0;i<k;) {
    dist=vvo[0][i]; targ=vvo[3][i]; 
    for (minind=i,j=i+1;vvo[3][j]==targ;j++) {
      if (vvo[0][j]<dist) { minind=j; dist=vvo[0][j]; } 
    }
    i=j;
    scr[kk++]=minind;
  }
...
static double slone (void* vv) {
  ...
  if (ifarg(3)) ni=(int)*getarg(3); else {
    ...
    ni=(int)(val-min)/(max-min)*(double)(nsrc-1); 
  }
  if (src[ni]<val) {
...
static double peak (void* vv) {
  ...
  n = vector_instance_px(vv, &x); 
  nc = vector_arg_px(1, &vc); 
  if (n==0) x=vector_newsize((IvocVect*)vv,n=100);
  if (ifarg(2)) y=vector_newsize(vy=vector_arg(2),ny=n); else ny=0; 
  if (vc[1]-vc[0]<0) up=0; else up=1;  
  last=vc[1];
...
static double negwrap (void* vv) {
  ...
  double *x, cnt, sig;
  n = vector_instance_px(vv, &x);
  if (ifarg(1)) sig = (int)*getarg(1); else sig=1e9; 
  if (sig==0.) {
...
static double v2p (void* vv) {
  ...
  for (i=0,j=1,cnt=0;ifarg(j) && i<n;i++,j++) {
    if (hoc_is_double_arg(j)) continue; 
    *hoc_pgetarg(j)=x[i];
    cnt++;
  }
  return (double)cnt;
...
static double fetch (void* vv) {
  ...
  if (hoc_is_object_arg(3)) {
    ...
    if (ny>pL->isz) vector_resize((IvocVect*)vector_arg(3),pL->isz); 
...
static double covar (void* vv) {
  ...
  n = vector_instance_px(vv, &x); 
  if (n==0) { pL = AllocListVec(*hoc_objgetarg(1)); 
  ...
  for (i=0;i<n;i++) for (j=0;j<m;j++) pL->pv[i][j] -= mean[j]; 
  for (j=0;j<m;j++) for (j2=j;j2<m;j2++) { 
...
static double vlxpose (void* vv) {
  ...
  for (j=0,k=0;j<c;j++,k++) for (i=0;i<c2;i++) { 
    if (k>=pL2->pbuflen[i]) { sz=pL2->pbuflen[i]; 
      sz=(sz<10)?100:(sz*2);
...
static double lcat (void* vv) {
  ...
  IvocVect *vw;
  n = vector_instance_px(vv, &x);
  vector_resize((IvocVect*)vv,maxsz=vector_buffer_size((IvocVect*)vv)); 
  ob1 = *hoc_objgetarg(1);
...
static double unq (void* vv) {
  ...
  n=vector_instance_px(vv, &x);
  y=vector_newsize((IvocVect*)vector_arg(1),n); 
...
int uniq2 (int n, double *x, double *y, double *z) {
  ...
  for (i=0,j=0;i<n;i++) { 
    ...
    if (i%10*cnt==0) { 
      for (k=0;k<cnt;k++) if (scr[k]) break; 
      if (k==cnt) break; 
    }
  } 
  return cnt;
...
static double rdmany (void* vv) {	
  ...
  i=2*sizeof(int) + 2*sizeof(double); 
  j=2*sizeof(int);  
  r=fread(&n,sizeof(int),2,f);
  vsz=n[0]; code=n[1];
  fseek(f,(long)-2*sizeof(int),SEEK_CUR);  
  if (DEBUG_VECST) printf("rdmanyDBA: %ld %d %d\n",ftell(f),vsz,code);
  switch (code) {
    
    case 2:sz=2; hd=i; break; 
    case 3:sz=4; hd=j; break; 
    case 4:sz=8; hd=j; break; 
  ...
  if (code==2) {
    ...
    for (last=-1,i=0;i<num;i++) {
      ...
      if (n[0]!=vsz){printf("rdmany ERRA vec(%d) %d vs %d\n",iflag?(int)ind[i]:i,vsz,n[0]);hxe();}
      if (n[1]!=code){printf("rdmany ERRB code mismatch %d %d\n",n[1],code);hxe();}
      r=fread(xs,sizeof(double),n[0],f); 
...
static double rdfile (void* vv) {	
  ...
  fseek(f,0,SEEK_END); sz=(int)ftell(f); rewind(f); 
  ...
  for (i=0,k=0,cnt=0;i<sz;cnt++) { 
    xi=(int*) (xc+i);
    vsz=xi[0];
    ty= xi[1]; i+=(2*sizeof(int)); 
    if (vsz<=0 || ty<1 || ty>5) {
...
FUNCTION ojtnum () {
  ...
  if (ifarg(2)) {
    ...
    for (ii=strlen(name);ii>1;ii--) if (name[ii]==91) {name[ii]=0; break;} 
