...
INITIAL {
    ...
    printf("naRsg>> the number of channels is %.0f.\n", Nna)
    printf("Area of the neuron = %.0f um^2.\n", area)
    set_seed(seed)
    VERBATIM
    init_cn();
...
int compute_sigmas(double v, int num_channels) {
    ...
    if (flag != 0) {
	printf("error on gsl_linalg_LU_decomp.\n");
	goto sigmas_end;
    }
    ...
    if (flag != 0) {
	printf("error on gsl_linalg_LU_solve.\n");
	goto sigmas_end;
    }
...
int compute_taus(double v) {
    ...
    if (flag != 0) {
	printf("error on gsl_eigen_nonsymm in compute_taus.\n");
	goto taus_end;
    }
...
int compute_coefficients(double v, int open_state, int num_channels) {
    ...
    gsl_eigen_nonsymm_params(1, 1, w);
    
    flag = gsl_eigen_nonsymmv(&A_view.matrix, &eigs_view.vector, &U_view.matrix, w);
    
    if (flag != 0) {
	printf("error on gsl_eigen_nonsymm in compute_coefficients.\n");
	goto coeff_end;
    }
    ...
    if (flag != 0) {
	printf("error on gsl_linalg_LU_invert.\n");
        goto coeff_end;
    }
