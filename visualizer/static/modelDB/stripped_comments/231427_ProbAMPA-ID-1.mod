...
PARAMETER {
        ...
        tau_r_AMPA = 0.2   (ms)  
        tau_d_AMPA = 1.7    (ms)  
        tau_r_NMDA = 0.00000001   (ms) 
        tau_d_NMDA = 0.00000002     (ms) 
        Use = 1.0   (1)   
        Dep = 100   (ms)  
        Fac = 10   (ms)  
        e = 0     (mV)  
	mg = 1   (mM)  
	mgVoltageCoeff = 0.08 (mV) 
        mggate
    	gmax = .001 (uS) 
    	u0 = 0 
}
...
BREAKPOINT {
	...
	mggate = 1 / (1 + exp(mgVoltageCoeff * -(v)) * (mg / 3.57 (mM))) 
        g_AMPA = (1) * gmax * (B_AMPA-A_AMPA) 
	g_NMDA = 0*gmax*(B_NMDA-A_NMDA) * mggate 
        i_AMPA = g_AMPA*(v-e) 
	i_NMDA = g_NMDA*(v-e) 
	i = i_AMPA + i_NMDA
}
