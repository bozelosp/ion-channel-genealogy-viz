...
PROCEDURE callback (fl) {
  ...
  while (ddel<=DELMIN) { 
    ...
    if(seadsetting==3 && !jp->inhib) wts[1]=jp->wgain[i]; else wts[1]=0.0; 
...
FUNCTION rddvi () {
  ...
  for(iCell=0;iCell<pg->cesz;iCell++){
    ...
    lop(pg->ce,iCell);
    int ret;
    ret = fread(&qp->id,sizeof(unsigned int),1,fp); 
    ret = fread(&qp->type,sizeof(unsigned char),1,fp); 
    ret = fread(&qp->col,sizeof(unsigned int),1,fp); 
    ret = fread(&qp->dead,sizeof(unsigned char),1,fp); 
    ret = fread(&qp->dvt,sizeof(unsigned int),1,fp); 
    ...
    for(i=0;i<qp->dvt;i++){
      ret = fread(&iOutID,sizeof(unsigned int),1,fp); 
    ...
    ret = fread(qp->del,sizeof(double),qp->dvt,fp); 
    qp->sprob = (unsigned char*)malloc(sizeof(unsigned char)*qp->dvt);
    ret = fread(qp->sprob,sizeof(unsigned char),qp->dvt,fp); 
  }
  printf("\n");
  return 1.0;
  ENDVERBATIM
...
static void applyplast (id0* ppo,double pospkt, double phase, double pinc) {
  ...
  for(i=0;i<ppo->econvsz;i++) {           
    prid = ppo->peconv[i];                
    if(pg->lastspk[prid]<0) continue;     
    d = pospkt - pg->lastspk[prid]; 
    if(d*phase<=0 || fabs(d)>maxplastt) continue; 
...
PROCEDURE jitcondiv () {
  ...
  pg->lastspk = (double*)calloc(pg->cesz,sizeof(double)); 
