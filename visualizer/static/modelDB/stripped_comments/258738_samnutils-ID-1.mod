...
FUNCTION SpikeTrainCoinc(){
  ...
  double dNumBins = dStimDur / dBinSize; 
  ...
  double dRandCoinc = (iSz1 * iSz2) / dNumBins; 
...
FUNCTION SpikeTrainEditDist(){
  ...
  for( row = 1; row < iSz1 + 1; row += 1){
    for(col = 1; col < iSz2 + 1; col += 1){
      dtab[row][col]= DMIN(  DMIN(dtab[row-1][col]+dDelCost,dtab[row][col-1]+dDelCost),     
                                              dtab[row-1][col-1]+dMoveCost*fabs(pVec1[row-1]-pVec2[col-1])); 
    }
  }
