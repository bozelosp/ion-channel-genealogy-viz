TITLE Kv12 channel
: Based on Rothman and Manis KLVT channela
: Structure of this mod file is by Jose Ambros-Ingerson
...
PARAMETER {
  ...
  tauG_n	= 0.34		: Left-right bias. range is (0,1)
  ...
  : tau_h parameters
  tauA_h	= 300	(ms)
  tauDv_h	= 0	(mV)	: Delta to vhalf_n
  tauG_h	= 0.34		: Left-right bias. range is (0,1)
  tauF_h	= 0		: Up-Down bias. range is ~ -3.5(cup-shape), -3(flat), 0(from k), 1(sharper)
  tau0_h	= 50 	(ms)	: minimum tau
...
BREAKPOINT {
  ...
  if( n >=0 ){	: make sure no domain error for pow. Cvode may test it
    g 	= gbar * n^gates_n * (sust + (1 - sust)*h)
  }
  i	= g * ( v - ek ) * (1e-4)
  ik	= i
}
