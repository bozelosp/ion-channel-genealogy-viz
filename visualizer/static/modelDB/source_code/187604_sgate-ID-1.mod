...
"To simulate a non-homogeneous Poisson process with intensity function Î»(t), 
choose a sufficiently large Î» so that Î»(t) = Î» p(t) and simulate a Poisson 
process with rate parameter Î». Accept an event from the Poisson simulation at 
