NEURON {
  ...
  RANGE taum,myinvl,oinvl,WINV,invlt         
...
CONSTRUCTOR {
  ...
  { int lid,lty;
    ...
    ip->invl = ip->record = ip->jitter = ip->input = 0; 
    ip->rve=-1;
  }
  ENDVERBATIM
}
...
PROCEDURE initinvl () {
  ...
  {int i, err=0;
    ...
    if (err) { invlmax=-1; invlp=nil; return(0.); }
  }
  ENDVERBATIM
}
...
FUNCTION shift (vl) { 
  ...
  if ((t<(invlt-myinvl)+myinvl/2) && invlt != -1) { 
    _lshift=0.;  
  } else {
    expand = -(_lvl-(-65))/20; 
