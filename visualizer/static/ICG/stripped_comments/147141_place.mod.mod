...
static double simts (void* vv) {
  ...
  while (0) switch (op) {
    ...
    case    'S':    ; 
    break;
    case    'X':    map_x = MAP_XSZ; 
    break;
    case    'Y':    map_y = MAP_YSZ; 
    break;
    default:
    instruct();
  }
  ...
  while (1) {
    if (fflag) {
      ...
      for(i = 0; i < xys[2]; i++) fread(&mt,sizeof(int), 1, ifp); 
      x = xys[0]; y = xys[1];
    } else {
    ...
    if (sflag) {
      if (s>0) {
        xx[q]=(double)(n_samps + drand48())*timestamp_units_per_samp*0.1; 
        yy[q]=(double)s;
        q++;
      }
    } else {
...
static double kendall (void* vv) {
  ...
  AnInt=AnalysisInterval*1e3; 
  MaxT=MaxTime*1e3; 
  MaxBins = (ui4) AnInt/BinSize;
  NumPairs = (ui4)MaxBins*(MaxBins-1); 
...
static double kendal2 (void* vv) {
  ...
  NumPairs = (ui4)MaxBins*(MaxBins-1)/2; 
  if(VERBOSE>1) printf("NumPairs=%d, MaxBins=%d\n",NumPairs,MaxBins);
  Dx = (si1 *)calloc(NumPairs, sizeof(si1)); 
...
static double kend2 (void* vv) {
  ...
  if (ifarg(2)) pw=1; else pw=0; 
  NumPairs = (ui4)(MaxBins-1); 
  Dx = (si1 *)calloc(NumPairs, sizeof(si1)); 
  Dy = (si1 *)calloc(NumPairs, sizeof(si1));
...
si4	GetASCIIEncoding(si1 *Encoding, si1 ***P)
{
	...
	*P = Parameters;
	...
	if((TerminateString = (si1 *)strstr(Parameters[0], ASCII_TERMINATING_MARK )) != NULL){	
		*TerminateString = '\0';
		return(1);
	}
	...
	while(sscanf(Line, "%s", String) != EOF){
		...
		if((TerminateString = (si1 *)strchr(Parameters[n], ')')) != NULL){ 
			*TerminateString = '\0' ;
			return(++n);
		}
		Line += (strlen(String) + 1);
	...
	dp = (ui1 *)SectionStart;
		...
		if(NumberOfParameters){
			for(i=0; i < NumberOfParameters; i++){
                                BPFEEGChannels[i] = atoi(Parameters[i]) - 1; 
			}
		}
    ...
    switch (type) {
      ...
      if ((ix=i)<npvec) { 
        if (!waveformflag) {
          ...
          if (vlen[ix]+SamplesPerWaveform+10>=iSz) {  
...
static double readbpfeeg(void* vv) {
  ...
  if(ifarg(3)) SamplesPerRecord = *getarg(3); 
  if(ifarg(4)) AdjustGain = (sf4) *getarg(4); 
  ...
  vvo = (double**)malloc(iMaxCols*sizeof(double*)); 
  if(!vvo){
  ...
  int iT = EEGChannels, iLim = iOffsetStart; 
  double tcur = 0.0, tinc = (1/2e3)/60.0;    
  for(i=0;i<iLim;i++)
  { vvo[iT][i] = tcur;
    tcur += tinc;
  }
...
FUNCTION bpfeegsamp(){
  ...
  fname = gargstr(1);
  if(ifarg(2)) SamplesPerRecord = *getarg(2); 
...
PROCEDURE install () {
  ...
  INSTALLED=1
VERBATIM
  install_vector_method("simts_PLACE", simts); 
  ...
  install_vector_method("glob", glob); 
  install_vector_method("cumul", cumul); 
