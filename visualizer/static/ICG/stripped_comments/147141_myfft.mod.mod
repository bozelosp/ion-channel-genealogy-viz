                         ...
                         int Vsz)
{
	...
	double *w = (double*) calloc(N,sizeof(double));		
        double *sinc = (double*) calloc(N,sizeof(double));	
...
double fftband(void* v){
  ...
  double dret = 0.;
  double* pout = 0;
  int outlen = vector_arg_px(1,&pout); 
  int lowhz = (int)*getarg(2); 
  int hihz = (int)*getarg(3); 
  int log2length = ceil(log((double)(n))/log(2.0));
  int fftlen = two_to_the((double)log2length);
  double* fftd = (double*)calloc(fftlen,sizeof(double)); 
  if(outlen  < fftlen){
