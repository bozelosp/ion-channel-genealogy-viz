...
static double slct (void* vv) {
  ...
  for (i=0,j=0,k=0;i<num;i++,j++) { 
    ...
    if (key[j]==EQW || key[j]==EQV) if (k==0){j--;k++;} else k=0; 
  }
  for (j=0;j<nk;j++) { 
    field[j]=-1;
  ...
  for (i=0,n=0;i<nk;i++) if (key[i]==EQV || key[i]==EQW) n++; 
  if (nk+n!=num) { 
...
static double updown (void* vv) {
  ...
  if (DEBUG_VECST && ifarg(4)) { dbn = vector_arg_px(4, &dbx); 
  ...
  for (i=0,k=0; i<nsrc; i++) if (tmp[i]<0.) {
    ...
    nq[3][k]=lt; 
    nq[4][k]=src[i]-lt; 
    nq[7][k]=(src[i]-src[i-(int)SHM_UPDOWN])-(src[i+(int)SHM_UPDOWN]-src[i]); 
    nq[8][k]=(double)k;  
    k++;
  }
