NEURON {
	...
	RANGE volin, volout, volglia, deltan, deltaglia 
	...
	POINTER diamg, inag, ikg, iclg, iag 
...
KINETIC state {
	...
	clflux[0] = -deltan*cl[0] - (icl*diam)*PI*(1e4)/FARADAY*-1 
	 kflux[0] = -deltan* k[0] - ( ik*diam)*PI*(1e4)/FARADAY
	 aflux[0] = -deltan* a[0]
	naflux[1] =  (deltan+deltag)*na[1] + (ina*diam+inag*diamg)*PI*(1e4)/FARADAY
	caflux[1] =  (deltan+deltag)*ca[1] + ((ica)*diam+0*diamg)*PI*(1e4)/(FARADAY*2)  
	clflux[1] =  (deltan+deltag)*cl[1] + (icl*diam+iclg*diamg)*PI*(1e4)/FARADAY*-1 
	...
	caflux[2] = 0
	clflux[2] = -deltag*cl[2] - (iclg*diamg)*PI*(1e4)/FARADAY*-1 
