...
def count_connections(G, thresholds, ion_class, ion_class_counts, node_dict):
    connection_counts = []
    for threshold in thresholds:
        ...
        connection_counts.append(G_threshold.number_of_edges())
...
for i, ion_class in enumerate(sorted_ion_classes):
    connection_counts = count_connections(G, thresholds, ion_class, ion_class_counts, node_dict)
    ax.plot(thresholds, connection_counts, marker='o', label=f'{ion_class} ({ion_class_counts[ion_class]} nodes)', linewidth=2, color=palette[i])

ax.set_xlabel('Similarity Threshold', fontsize=14, labelpad=20)
ax.set_ylabel('Number of Connections', fontsize=14, labelpad=20)
