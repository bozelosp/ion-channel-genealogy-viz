...
for i, ion_class in enumerate(sorted_ion_classes):
    plot_centrality_measure(G, thresholds, ax, ion_class, ion_class_counts, nx.betweenness_centrality, 'Betweenness Centrality', palette[i], node_dict)

ax.set_xlabel('Similarity Threshold', fontsize=14, labelpad=20)
ax.set_ylabel('Betweenness Centrality', fontsize=14, labelpad=20)
...
ax.set_title("Betweenness Centrality vs Similarity Thresholds", fontsize=18, fontweight='bold', pad=20)

# Adjust the layout to make space for the legend and increase margins
plt.tight_layout()
plt.subplots_adjust(right=0.75, top=0.9, bottom=0.15)

fname = 'betweenness_centrality.png'
