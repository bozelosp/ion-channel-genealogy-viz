...
def count_provincial_hubs(G, thresholds, ion_class, ion_class_counts, node_dict):
    provincial_hub_counts = []
    for threshold in thresholds:
        ...
        provincial_hub_counts.append(num_provincial_hubs)

    return provincial_hub_counts
...
for i, ion_class in enumerate(sorted_ion_classes):
    provincial_hub_counts = count_provincial_hubs(G, thresholds, ion_class, ion_class_counts, node_dict)
    ax.plot(thresholds, provincial_hub_counts, marker='o', label=f'{ion_class} ({ion_class_counts[ion_class]} nodes)', linewidth=2, color=palette[i])

ax.set_xlabel('Similarity Threshold', fontsize=14, labelpad=20)
ax.set_ylabel('Number of Provincial Hubs', fontsize=14, labelpad=20)
...
ax.set_title("Number of Provincial Hubs vs Similarity Thresholds", fontsize=18, fontweight='bold', pad=20)

# Adjust the layout to make space for the legend and increase margins
plt.tight_layout()
plt.subplots_adjust(right=0.75, top=0.9, bottom=0.15)

fname = 'provincial_hubs.png'
