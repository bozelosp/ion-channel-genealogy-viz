...
def count_connector_hubs(G, thresholds, ion_class, ion_class_counts, node_dict):
    connector_hub_counts = []
    for threshold in thresholds:
        ...
        num_connector_hubs = sum(1 for node in G_threshold.nodes() if degrees[node] > 1 and betweenness[node] > np.mean(list(betweenness.values())))
        connector_hub_counts.append(num_connector_hubs)

    return connector_hub_counts
...
for i, ion_class in enumerate(sorted_ion_classes):
    connector_hub_counts = count_connector_hubs(G, thresholds, ion_class, ion_class_counts, node_dict)
    ax.plot(thresholds, connector_hub_counts, marker='o', label=f'{ion_class} ({ion_class_counts[ion_class]} nodes)', linewidth=2, color=palette[i])

ax.set_xlabel('Similarity Threshold', fontsize=14, labelpad=20)
ax.set_ylabel('Number of Connector Hubs', fontsize=14, labelpad=20)
...
ax.set_title("Number of Connector Hubs vs Similarity Thresholds", fontsize=18, fontweight='bold', pad=20)

# Adjust the layout to make space for the legend and increase margins
plt.tight_layout()
plt.subplots_adjust(right=0.75, top=0.9, bottom=0.15)

fname = 'connector_hubs.png'
