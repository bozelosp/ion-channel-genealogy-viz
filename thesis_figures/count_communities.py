...
def count_communities(G, thresholds, ion_class, ion_class_counts, node_dict):
    community_counts = []
...
for i, ion_class in enumerate(sorted_ion_classes):
    community_counts = count_communities(G, thresholds, ion_class, ion_class_counts, node_dict)
    ax.plot(thresholds, community_counts, marker='o', label=f'{ion_class} ({ion_class_counts[ion_class]} nodes)', linewidth=2, color=palette[i])

ax.set_xlabel('Similarity Threshold', fontsize=14, labelpad=20)
ax.set_ylabel('Number of Communities', fontsize=14, labelpad=20)
...
ax.set_title("Number of Communities vs Similarity Thresholds", fontsize=18, fontweight='bold', pad=20)

# Adjust the layout to make space for the legend and increase margins
plt.tight_layout()
plt.subplots_adjust(right=0.75, top=0.9, bottom=0.15)

fname = 'count_communities.png'
