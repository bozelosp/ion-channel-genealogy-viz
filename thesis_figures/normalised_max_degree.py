...
def calculate_max_degree_normalized(G, thresholds, ion_class, ion_class_counts, node_dict):
    ...
    for threshold in thresholds:
        ...
        num_provincial_hubs = sum(1 for degree in degrees.values() if degree > 1)
        ...
        if num_provincial_hubs > 0:
            max_degree_normalized.append(max_degree / num_provincial_hubs)
        else:
            max_degree_normalized.append(0)

    return max_degree_normalized
...
for i, ion_class in enumerate(sorted_ion_classes):
    max_degree_normalized = calculate_max_degree_normalized(G, thresholds, ion_class, ion_class_counts, node_dict)
    ax.plot(thresholds, max_degree_normalized, marker='o', label=f'{ion_class} ({ion_class_counts[ion_class]} nodes)', linewidth=2, color=palette[i])

ax.set_xlabel('Similarity Threshold', fontsize=14, labelpad=20)
ax.set_ylabel('Max Degree / Number of Provincial Hubs', fontsize=14, labelpad=20)
...
ax.set_title("Max Degree / Number of Provincial Hubs vs Similarity Thresholds", fontsize=18, fontweight='bold', pad=20)

# Adjust the layout to make space for the legend and increase margins
plt.tight_layout()
plt.subplots_adjust(right=0.75, top=0.9, bottom=0.15)

fname = 'normalised_max_degree.png'
