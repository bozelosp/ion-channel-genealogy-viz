...
def plot_clustering_coefficient(G, thresholds, ax, ion_class, ion_class_counts, color, node_dict):
    ...
    ax.plot(thresholds, clustering_coefficients, marker='o', label=f'{ion_class} ({ion_class_counts[ion_class]} nodes)', linewidth=2, color=color)
...
for i, ion_class in enumerate(sorted_ion_classes):
    plot_clustering_coefficient(G, thresholds, ax, ion_class, ion_class_counts, palette[i], node_dict)

ax.set_xlabel('Similarity Threshold', fontsize=14, labelpad=20)
ax.set_ylabel('Clustering Coefficient', fontsize=14, labelpad=20)
...
ax.set_title("Clustering Coefficient vs Similarity Thresholds", fontsize=18, fontweight='bold', pad=20)

# Adjust the layout to make space for the legend and increase margins
# plt.tight_layout()
plt.subplots_adjust(right=0.75, top=0.9, bottom=0.15)

fname = 'clustering_coefficient.png'
