...
def plot_centrality_measure(G, thresholds, ax, ion_class, ion_class_counts, centrality_func, centrality_name, color, node_dict):
    ...
    for threshold in thresholds:
        ...
        try:
            centrality = centrality_func(G_threshold, max_iter=2000)
...
for i, ion_class in enumerate(sorted_ion_classes):
    plot_centrality_measure(G, thresholds, ax, ion_class, ion_class_counts, nx.eigenvector_centrality, 'Eigenvector Centrality', palette[i], node_dict)

ax.set_xlabel('Similarity Threshold', fontsize=14, labelpad=20)
ax.set_ylabel('Eigenvector Centrality', fontsize=14, labelpad=20)
...
ax.set_title("Eigenvector Centrality vs Similarity Thresholds", fontsize=18, fontweight='bold', pad=20)

# Adjust the layout to make space for the legend and increase margins
plt.tight_layout()
plt.subplots_adjust(right=0.75, top=0.9, bottom=0.15)

fname = 'eigenvector_centrality.png'
