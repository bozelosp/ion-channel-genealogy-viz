...
for i, threshold in enumerate(thresholds):
    ax = axs[i]
    mask = np.triu(np.ones_like(all_similarity_matrices[i], dtype=bool))
    sns.heatmap(all_similarity_matrices[i], annot=True, fmt='d', cmap='YlGnBu', vmin=global_min, vmax=global_max,
                cbar_kws={'label': 'Number of Similar Models'}, ax=ax, mask=mask)
    ...
    ax.set_xlabel("Target Ion Channel Class", labelpad=14)
    ax.set_ylabel("Source Ion Channel Class", labelpad=14)
...
fname = 'similarity_matrices_lower_triangle.png'
